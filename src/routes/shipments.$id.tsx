...
function ShipmentDetail() {
  ...
  if (shipment === null)
    return (
      <div className="p-8 text-muted-foreground text-sm">
        Shipment not found.{" "}
        <Link to="/shipments" className="text-primary">
          Back
        </Link>
      </div>
    );
  ...
  const etaDate =
    etaHours !== null ? new Date(Date.now() + etaHours * 3600_000) : new Date(shipment.etaAt);
  ...
  const submitPod = async () => {
    ...
    await addProofOfDelivery(shipment.id, {
      receivedBy,
      photoDataUrl: photo,
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 max-w-[1600px] mx-auto">
      <div className="flex items-center gap-3">
        <Link to="/shipments" className="text-muted-foreground hover:text-foreground">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">
            {shipment.origin.name} → {shipment.destination.name} · {carrier?.name}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <Stat
          icon={Clock}
          label="Dynamic ETA"
          value={format(etaDate, "MMM d · HH:mm")}
          sub={formatDistanceToNow(etaDate, { addSuffix: true })}
        />
        <Stat
          icon={DollarSign}
          label="Cost"
          value={`$${shipment.costUsd.toFixed(0)}`}
          sub={`${shipment.distanceKm.toFixed(0)} km`}
        />
      </div>
        ...
        <Card className="p-4 bg-card/60">
          ...
          {shipment.status !== "delivered" && shipment.status !== "exception" && (
            <div className="mt-4 pt-4 border-t border-border">
              <div className="text-[11px] uppercase tracking-wider text-muted-foreground mono mb-2">
                Quick actions
              </div>
              <div className="flex flex-wrap gap-2">
                <Button size="sm" variant="secondary" onClick={() => advance("in_transit")}>
                  Mark in transit
                </Button>
                <Button size="sm" variant="secondary" onClick={() => advance("out_for_delivery")}>
                  Out for delivery
                </Button>
              </div>
            </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-2 p-4 bg-card/60">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[11px] mono text-muted-foreground">
              total: {shipment.skus.reduce((a, s) => a + s.weightKg, 0).toFixed(1)} kg · $
              {shipment.skus.reduce((a, s) => a + s.value, 0).toLocaleString()}
            </span>
          </div>
            ...
            <tbody>
              {shipment.skus.map((s) => (
                <tr key={s.id} className="border-t border-border">
                  <td className="py-2.5">
                    {s.name}
                    <div className="text-[11px] text-muted-foreground mono">{s.id}</div>
                  </td>
            ...
            <div className="mt-4 p-3 rounded-md border border-info/30 bg-info/5 flex items-center gap-3 text-sm">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  Threshold {shipment.tempThresholdC}°C — within tolerance
                </div>
              </div>
            </div>
          )}
        ...
        <Card className="p-4 bg-card/60">
          ...
          <Button
            ...
            onClick={() =>
              toast.success(`Notification queued to ${shipment.customer.email}`, {
                description: "Twilio/Resend webhook stub",
              })
            }
          >
        ...
        <Card className="p-4 bg-card/60">
          ...
          {shipment.pod?.at ? (
            <div className="space-y-2 text-sm">
              <div>
                Received by <b>{shipment.pod.receivedBy}</b>
              </div>
              <div className="text-xs text-muted-foreground mono">
                {format(new Date(shipment.pod.at), "PPpp")}
              </div>
              {shipment.pod.photoDataUrl && (
                <img
                  src={shipment.pod.photoDataUrl}
                  alt="pod"
                  className="rounded-md border border-border max-h-48"
                />
              )}
              {shipment.pod.signatureDataUrl && (
                <div>
                  <div className="text-[11px] uppercase mono text-muted-foreground mb-1">
                    Signature
                  </div>
                  <img
                    src={shipment.pod.signatureDataUrl}
                    alt="sig"
                    className="rounded-md border border-border bg-white max-h-32"
                  />
                </div>
              )}
            </div>
            ...
            <div className="space-y-3">
              <Input
                placeholder="Received by (full name)"
                value={receivedBy}
              ...
              <label className="flex items-center gap-2 px-3 py-2 rounded-md border border-dashed border-border cursor-pointer hover:border-primary/40 text-sm">
                ...
                <span className="text-muted-foreground">
                  {photo ? "Photo attached" : "Attach delivery photo"}
                </span>
                <input
                  type="file"
                  accept="image/*"
              ...
              {photo && (
                <img
                  src={photo}
                  alt="preview"
                  className="rounded-md border border-border max-h-32"
                />
              )}
              <div>
                <div className="text-[11px] uppercase mono text-muted-foreground mb-1 flex items-center gap-1">
                ...
                <button
                  onClick={() => sigRef[0]?.clear()}
                  className="text-[11px] mono text-muted-foreground hover:text-foreground mt-1"
                >
                  clear
                </button>
              </div>
              <Button onClick={submitPod} className="w-full">
                Confirm delivery
              </Button>
            </div>
          )}
              ...
              <li key={a.id} className="text-xs border-l-2 border-border pl-3 py-1">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="mono text-muted-foreground">
                    {format(new Date(a.at), "MMM d HH:mm:ss")}
                  </span>
                </div>
